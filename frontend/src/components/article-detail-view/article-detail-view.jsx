...
const ProductDetailView = ({ product, receivedTypes, state }) => {
  const updateArticle = async (id, data) => {
    try {
      const response = await axios.put(`/update_article/${id}`, data, {
        headers: {
      ...
      Swal.fire("Reussi !", response.data.message, "success");
    } catch (err) {
      console.error(err);
      Swal.fire(
        "Erreur",
  ...
  // close form
  const closeForm = () =>
    (document.querySelector("#product-detail-view").style.visibility =
      "hidden");
  ...
  const handleSubmit = (e) => {
    ...
    console.log(formData.getAll("imagesUrls"));
          ...
          <div className="images">
            ...
            <div className="other-view">
              {listImages.map((img, index) => (
                <div
                  key={index}
                  className={`img ${showImage === img ? "active" : ""}`}
                  onClick={() => {
                    deleteImage(img);
                  }}
                  style={{
                    backgroundImage: `url(http://localhost:3000/img/${img}), url(${img})`,
                    backgroundPosition: "center",
                    backgroundSize: "cover",
                    backgroundRepeat: "no-repeat",
                  }}
